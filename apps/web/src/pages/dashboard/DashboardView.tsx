...
export function DashboardView({
  ...
  return (
    <div>
            ...
            <tbody>
              {chantiers.map((c, i) => (
                <tr
                  key={c.id}
                  style={{
                    borderBottom: i < chantiers.length - 1 ? `1px solid var(--color-border)` : undefined,
                  }}
                >
                  <td className="px-4 py-3">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <p className="font-medium" style={{ color: "var(--color-text)" }}>{c.adresseRue}</p>
                  ...
                  <td className="px-4 py-3">
                    {c.surfaceM2 != null ? (
                      <span className="flex items-center gap-1 text-xs" style={{ color: "var(--color-text)", fontFamily: "var(--font-mono)" }}>
                        <Maximize2 size={11} style={{ color: "var(--color-muted)" }} />
                        {c.surfaceM2} m²
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      >
                        <Archive size={13} style={{ color: "var(--color-muted)" }} />
                      </button>
                    </div>
                  </td>
                </tr>
      ...
      {/* Dialog Modification */}
      <Dialog open={editingChantier !== null} onOpenChange={(open) => { if (!open) onCloseEdit(); }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle style={{ fontFamily: "var(--font-heading)", fontWeight: 600 }}>
      ...
      <AlertDialog open={archivingId !== null} onOpenChange={(open) => { if (!open) onCancelArchive(); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Archiver ce chantier ?</AlertDialogTitle>
            <AlertDialogDescription>
              Le chantier sera archivé et n'apparaîtra plus dans la liste. Cette action peut être inversée en contactant le support.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={onCancelArchive}>Annuler</AlertDialogCancel>
            <AlertDialogAction
