  ...
  try {
    ...
    for (const entry of entries) {
      if (entry.retryCount >= MAX_RETRY) {
        console.log(`[outbox] abandoned ${entry.entityType}/${entry.operation}/${entry.entityId} (max retry)`);
        db.update(outbox).set({ status: "abandoned" as OutboxStatus }).where(eq(outbox.id, entry.id)).run();
        continue;
      }

      const delay = Math.min(Math.pow(2, entry.retryCount) * 1000, MAX_BACKOFF_MS);
      ...
      try {
        ...
        console.log(`[outbox] synced ${entry.entityType}/${entry.operation}/${entry.entityId}`);
      } catch (err) {
        const status = (err as { status?: number }).status ?? 0;
        // 409 Conflict = idempotence : l'entite existe deja cote serveur avec ce UUID -> on considere synced
        if (status === 409) {
          console.log(`[outbox] 409 -> synced ${entry.entityType}/${entry.operation}/${entry.entityId}`);
          db.update(outbox).set({ status: "synced" as OutboxStatus }).where(eq(outbox.id, entry.id)).run();
          continue;
        }
        const isClientError = status >= 400 && status < 500;
        const nextStatus: OutboxStatus = isClientError ? "abandoned" : "pending";
        console.log(`[outbox] error ${entry.entityType}/${entry.operation}/${entry.entityId} status=${status} -> ${nextStatus}`);
