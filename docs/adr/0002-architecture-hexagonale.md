...
- **Pas de logique métier** : gestion de clients, adresses, tags. Ces contextes sont des _conteneurs de données référencés_ — validation triviale, zéro transition d'état, zéro règle de calcul. Il n'y a rien à isoler du framework parce qu'il n'y a pas de domaine à protéger.
...
- La légèreté sur les _vrais_ CRUD évite la cérémonie sans valeur — ce n'est pas un compromis de démarrage, c'est le bon niveau d'architecture pour des entités sans logique métier
